...
def main(args, config, model_folder, mode, verbose=True):
    ...
    for version in range(versions):
        ...
        for idx, fold in enumerate(folds):
            ...
            if mode == "val":
                feature_dataset = FeatureDatasetHDF5(
                    data_dir=os.path.join(config.dataset.val_folder, f'{idx}_fold', 'val'),
                    data_cols=config.dataset.data_cols,
                    base_label=config.dataset.base_label
                )
            ...
            if config.model.classifier == "clam":
                ...
                model = TransMIL_Features_PL.load_from_checkpoint(model_ckpt, strict=False)
                ...
                model = AttentionDeepMIL_PL.load_from_checkpoint(model_ckpt, strict=False)
            ...
            model.test_metrics = get_metrics(
                config,
                n_classes=config.dataset.num_classes if config.dataset.num_classes > 2 else 1,
            ...
            if mode == "val":
                ...
                results.append(trainer.validate(model, dataloaders=data, ckpt_path=None, verbose=verbose))
                tmp_predictions = trainer.predict(model, dataloaders=data, ckpt_path=None)
            elif mode == "test":
                print("Testing model.")
                results.append(trainer.test(model, dataloaders=data, ckpt_path=None, verbose=verbose))
                tmp_predictions = trainer.predict(model, dataloaders=data, ckpt_path=None)
            elif mode == "predict":
                tmp_predictions = trainer.predict(model, dataloaders=data, ckpt_path=None)

            tmp_predictions = [
                [item[0].detach().cpu().numpy().squeeze(), item[1].detach().cpu().numpy().squeeze().astype(np.uint8), item[2][0]] for item in tmp_predictions
            ]
            tmp = pd.DataFrame(tmp_predictions, columns=["preds", "label", "slide_name"])
        ...
        if mode in ["val", "test"]:
            ...
            df.to_csv(os.path.join(
                vo_folder,
                "{}_metrics_version_{}.csv".format(mode, version)
            ), index=False)
            df.describe().to_csv(os.path.join(
                vo_folder,
                "summary_{}_metrics_version_{}.csv".format(mode, version)
            ), index=True)

    if mode in ["val", "test"]:
        overall_df.to_csv(os.path.join(
            o_folder,
            "{}_metrics_overall.csv".format(mode)
        ), index=False)
        overall_df.describe().to_csv(os.path.join(
            o_folder,
            "summary_{}_metrics_overall.csv".format(mode)
        ), index=True)
